...
public:
    ...
    Currency(double num) {
        try {
            double temp = num * 100;
            int wholeTemp = static_cast<int>(temp/100);
            int fractionalTemp = static_cast<int>((temp - (wholeTemp * 100)) + 0.5);
            setWholePart(wholeTemp);
            setFractionalPart(fractionalTemp);
            if (num >= 0) {
    ...
    */
    ...
    double getValue() {//added getValue() method, as every operation required the value 
        return static_cast<double>(wholePart) + static_cast<double>(fractionalPart) / 100.0;
    }
